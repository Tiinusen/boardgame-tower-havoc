...
for d in expected_dirs:
    ...
    if not (p/'setup'/'004-initial-state.trace.md').exists():errs.append(d+': missing initial state')
...
for p in S.rglob('*.trace.md'):
    ...
    if p.name!='001-runtime-simulation-corpus.topic.trace.md' and 'TH-SIM-' not in t:
