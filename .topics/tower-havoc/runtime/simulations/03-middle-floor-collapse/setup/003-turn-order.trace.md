# Continuity Context

- Envelope Schema: [tiinex.root.v1](https://github.com/Tiinex/docs/blob/3988951208eb9a8926e84ab42625d4b42fa00c2d/.topics/.schemas/tiinex.root.v1.schema.md)
- Parent
  - Parent Schema: [towerhavoc.match.v1](../../../../../.schemas/tower-havoc/towerhavoc.match.v1.schema.md)
  - Created At: 2026-09-18 11:35:00
  - Trace: [TH-SIM-03 Middle Floor Hit Causes Upper Collapse](../002-match.match.trace.md)
  - Origin:
    - [relative](../002-match.match.trace.md)
- Current
  - Current Schema: [towerhavoc.turn-order.v1](../../../../../.schemas/tower-havoc/towerhavoc.turn-order.v1.schema.md)
  - Created At: 2026-09-18 11:35:00
  - Authors: ChatGPT

---

# TH-SIM-03 Turn Order

## Match Binding
- Match Artifact: ../002-match.match.trace.md
- Match ID: TH-SIM-03

## Eligible Seats
- Seats: seat-01; seat-02

## Ordering Method
- Method: scenario fixture

## Ordered Seats
- Order: seat-01; seat-02

## Finality
- State: accepted

## Interpretation Limits
- Does Not Prove: random fairness or real participant agreement
- Must Not Be Inferred: this ordering method changes canonical Tower Havoc setup outside the scenario

---

# Continuity Integrity

- [sha256-base64url-c14n-v2](https://github.com/Tiinex/docs/blob/3988951208eb9a8926e84ab42625d4b42fa00c2d/.topics/.validators/sha256-base64url-c14n-v2.validator.md)
  - Towards: [TH-SIM-03 Middle Floor Hit Causes Upper Collapse](../002-match.match.trace.md)
  - Value: 18Q7spdWdOEM6YDMV84SzLXCPjelAQYNh_H1kxfRhBk

- [sha256-base64url-c14n-v2](https://github.com/Tiinex/docs/blob/3988951208eb9a8926e84ab42625d4b42fa00c2d/.topics/.validators/sha256-base64url-c14n-v2.validator.md)
  - Towards: self
  - Value: mUUsDL6nekmCTARkpAiLqZk7D2zK5COfUWG2JgcezOU
